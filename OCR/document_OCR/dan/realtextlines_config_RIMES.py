...
from basic.transforms import aug_config, aug_config_real
...
dataset_name = "RIMES"  # ["IAM", "READ_2016", "RIMES"]
...
params = {
    "dataset_params": {
        ...
        "worker_per_gpu": 0, # 4,  # Num of parallel processes per gpu for data loading
        "config": {
            "max_size": {
                "max_width": 1232,
                "max_height": 64,
            },
            "full_compatibility" : True,  # whether to keep all the original samples of the dataset without cropping or not (except for curriculum learning cropping)
        ...
        "line_dataset_params": {
           "path": f"{Path.home()}/dev/python/formatted/RIMES_non_syn_line",
        },
    },
    ...
    "model_params": {
        ...
        "transfer_learning": {
            ...
            #"encoder": ["encoder", "outputs/FCN_RIMES_line_syn/checkpoints/best.pt", True, True],
            #"decoder": ["decoder", "outputs/FCN_RIMES_line_syn/checkpoints/best.pt", True, False],
            "encoder": ["encoder", f"{Path.home()}/dev/python/DAN/outputs/FCN_RIMES_100/checkpoints/best.pt", True, True],          
        },
    ...
    "training_params": {
        "output_folder": "dan_RIMES_page_real_20251201",  # folder name for checkpoint and results dan_IAM_page, dan_read_page, dan_rimes_page
        "max_nb_epochs": 500,  # maximum number of epochs before to stop
        ...
        "freeze_prefixes": None,  # list of prefixes of model parameters to freeze during some epochs (ex: ["encoder."])
        "optimizers": {
            "all": {
                "class": Adam,
                "args": {
